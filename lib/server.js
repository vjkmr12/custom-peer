...
app._configureWS = function(socket, key, id, token) {
  ...
  socket.on("message", function(data) {
    try {
      ...
      } else if (message.type === 'helloworld') {
        socket.emit("latency",{ message: 'A  joined!' })
        socket.send(JSON.stringify({ type: "latency" }));
      } else {
  ...
  //   socket.send(
  //     JSON.stringify({ type: "hello", payload: { msg: "hit latency" } })
  //   );
  //   cb(startTime);
  // });
